    ...
    if len(children) > 0:
        for child in children:
            ...
            joint = Joint(parent=root,
                          ...
                          observation_dim = nn_def[joint_mjcf.name]['sensor'],
                          output_dim = nn_def[joint_mjcf.name]['out'],
                          node_function = nn.Identity)
...
class RecursiveNN(nn.Module):
    ...
    def forward_curr_link(self,link : Link, x : dict) -> torch.Tensor:
        ...
        if (link.name == self.root_name and link.observation_dim > 0):
            feat_curr_link.append( x[link.name] )

        # from all joints'
        for joint in link.children:
            if joint.observation_dim > 0:
                joint_feat = x[joint.name]
    ...
    def obs_to_dict(self,
        ...
        if len(obs.shape) > 1:
            dict_obs[self.root_name] = np.zeros((obs.shape[0], self.robot.observation_dim))
        else:
            dict_obs[self.root_name] = np.zeros((self.robot.observation_dim))

        root_idx = 0
        if len(obs.shape) > 1:
            for i in range(obs_num):
                if self.state_list[i] in self.nn_def:
                    dict_obs[self.state_list[i]] =  obs[:, i]
                else:
                    dict_obs[self.root_name][:,root_idx] = obs[:, i]
                    root_idx += 1
        else:
            for i in range(obs_num):
                if self.state_list[i] in self.nn_def:
